...
function generateQuestion() {
  ...
  let a, b, result, missing;
  switch (op) {
    case '+':
      a = getRandomInt(1, 50); b = getRandomInt(1, 50); result = a + b; missing = getRandomInt(0, 1) ? a : b; break;
    case '-':
      a = getRandomInt(10, 99); b = getRandomInt(1, a); result = a - b; missing = getRandomInt(0, 1) ? a : b; break;
    case '×':
      a = getRandomInt(2, 12); b = getRandomInt(2, 12); result = a * b; missing = getRandomInt(0, 1) ? a : b; break;
    case '÷':
      b = getRandomInt(2, 12); result = getRandomInt(2, 12); a = b * result; missing = getRandomInt(0, 1) ? a : b; break;
    default:
      a = b = result = missing = 0;
  }
  return { a, b, op, result, missing };
}
...
const FillResult = () => {
  ...
  const getDisplay = () => {
    const { a, b, op, result, missing } = question;
    if (missing === a) return `__ ${op} ${b} = ${result}`;
    if (missing === b) return `${a} ${op} __ = ${result}`;
    return '';
  };
  ...
  const handleSubmit = e => {
    e.preventDefault();
    let correct = false;
    if (parseInt(input) === question.missing) {
      setScore(score + 1);
      ...
      setFeedback(t('minigame.incorrect', { answer: question.missing }));
      setStreak(0);
    }
    setTimeout(() => {
      setFeedback('');
  ...
  return (
    <div className="minigame-bg minigame-math">
      ...
      <div className="minigame-card mx-auto text-center animate__animated animate__fadeInDown">
        ...
        <h4>{t('minigame.complete_result')}</h4>
      <h2 className="mb-4">{getDisplay()}</h2>
      <form onSubmit={handleSubmit} className="mb-3">
        <input
          type="number"
          className="form-control form-control-lg mb-2 text-center"
            placeholder={t('minigame.missing_number')}
          value={input}
